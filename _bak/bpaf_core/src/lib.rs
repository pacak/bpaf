...
mod named {
        ...
        <T as FromStr>::Err: std::error::Error,
    {
        fn run<'a>(&'a self, ctx: Ctx<'a>) -> Fragment<'a, T> {
            ...
            let c = ctx.clone();
            let action = Box::new(move |arg: Option<(Name, Option<OsOrStr>)>| {
                let ctx = c.clone();
                exit.exit_task(if let Some((name, param)) = arg {
                    let val = match param {
                        Some(v) => {
                            c.advance(1);
                            v
                        ...
                        None => {
                            match ctx.args.get(ctx.cur() + 1) {
                                Some(val) => {
                                    if val.is_named() {
                                        return exit.exit_task(Err(Error::new(
                                            crate::error::Message::ArgNeedsValueGotNamed {
                                                name: name.to_owned(),
                                                meta: self.meta,
                                                val: val.to_owned(),
                                            },
                                ...
                                None => {
                                    return exit.exit_task(Err(Error::new(
                                        crate::error::Message::ArgNeedsValue {
                                            name: name.to_owned(),
                                            meta: self.meta,
                                        },
                                        ctx.cur(),
                    ...
                    val.parse::<T>().map_err(|e| Error::new(e, ctx.cur()))
                } else {
                    Err(Error::missing(
        ...
        T: Clone + 'static,
    {
        fn run<'a>(&'a self, ctx: Ctx<'a>) -> Fragment<'a, T> {
            ...
            let c = ctx.clone();
            let action = Box::new(move |arg: Option<(Name, Option<OsOrStr>)>| {
                exit.exit_task(if let Some((name, val)) = arg {
                    if val.is_none() {
                        c.advance(1);
                        Ok(self.present.clone())
                    } else {
                        todo!("{name:?} takes no value, got {val:?}")
                    }
                } else if let Some(fallback) = self.absent.as_ref() {
                    Ok(fallback.clone())
                } else {
                    Err(Error::missing(self.named.missing(None), c.cur()))
                })
            });
    ...
    impl Name<'_> {
        ...
        pub(crate) fn long(name: &str) -> Name {
...
mod positional {
        ...
        <T as std::str::FromStr>::Err: std::fmt::Display,
    {
        fn run<'a>(&'a self, ctx: Ctx<'a>) -> crate::executor::Fragment<'a, T> {
            let (exit, join) = ctx.fork();
            let c = ctx.clone();
            let action = Box::new(move |arg: Option<OsOrStr>| {
                println!("Positional parser got {arg:?}!!!!!!!!");
                let ctx = c.clone();
                let res = if let Some(val) = arg {
                    ctx.advance(1);
                    val.parse::<T>().map_err(|e| Error::new(e, ctx.cur()))
                } else {
                    Err(Error::missing(
