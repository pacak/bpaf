...
impl<T: 'static> Parser<T> for Anything<T> {
    fn run<'a>(&'a self, ctx: Ctx<'a>) -> Fragment<'a, T> {
        let (exit, join) = ctx.spark();

        let c = ctx.clone();
        let action = Box::new(move |marg: Option<&OsOrStr>| {
            let ctx = c.clone();
            match marg {
                Some(arg) => match (self.check)(arg.as_ref()) {
                    Some(v) => {
                        println!("Managed to parse the result!");
                        ctx.advance(1);
                        exit.exit_task(Ok(v));
                        true
                    }
    ...
    T: Clone + 'static,
{
    fn run<'a>(&'a self, ctx: Ctx<'a>) -> Fragment<'a, T> {
        Box::pin(async move {
            ...
            // keep collecting results as they are being produced. If parsers consume the same
            // amount - earlier one should win, otherwise one that consumes more wins
            while !fut.handles.is_empty() {
                println!("============= There are {} alt awaiting", fut.handles.len());
                let next = (&mut fut).await;
                match &next {
                    Ok(_) => println!("++++++++++++ Got result"),
                    Err(e) => println!("++++++++++++ Got error {e:?}"),
                };

                res = match (res, next) {
                    (Ok((offset1, val1)), Ok(val2)) => {
                        let offset2 = ctx.cur();
                        println!("Old: {offset1:?}, new: {offset2:?}");
                        if offset1 >= offset2 {
                            Ok((offset1, val1))
                        } else {
                            Ok((offset2, val2))
                    ...
                    (ok @ Ok(_), Err(_)) => ok,
                    (Err(_), Ok(res)) => Ok((ctx.cur(), res)),
                    (Err(e1), Err(e2)) => Err(e1.combine_with(e2)),
                };
            }
            res.map(|(_offset, val)| val)
        })
    }
