...
fn get_any_simple() {
    ...
    let b = any("REST", |s: &OsStr| Some(s.to_os_string())).help("any help");
...
fn get_any_many() {
    let a = short('a').switch();
    let b = any("REST", |s: &OsStr| Some(s.to_os_string()))
        .help("any help")
        .many();
...
#[test]
fn get_any_many2() {
    let parser: OptionParser<Vec<OsString>> = any("REST", |s: &OsStr| Some(s.to_os_string()))
        .many()
        .to_options();
