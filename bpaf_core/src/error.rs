...
#[derive(Debug)]
pub(crate) struct CompValue {
    /// Name and adjacency type, if applicable.
    ///
    /// Set if we are completing a named value via `--foo=VAL`, `-f=VAL` or `-fVAL`
    /// Unset for positional items and separate named items (`--foo VAL`)
    ///
    /// So literally `--name[=]`, if present
    pub(crate) name: Option<Box<str>>,
    /// What user typed so far.
    ///
    /// Both name and value don't have to be Boxed, but this helps to keep the variant size low?
    pub(crate) value: Box<std::ffi::OsStr>,
...
impl Error {
    pub(crate) fn combine(self, e2: Error) -> Error {
        match (self, e2) {
            ...
            (Error::CompValue(v), e) => Error::CompReply(v.into_reply()).combine(e),
            (e, Error::CompValue(v)) => e.combine(Error::CompReply(v.into_reply())),
