...
fn test_adjacent() {
    ...
    let parser = short('a').nest((b, c)).many().to_options();

    let r = parser.run_inner("-a -c -a -b -a -b -c").unwrap();
...
fn adjacent_error_message_arg_single() {
    ...
    let d = short('d').switch();
    let adj = short('a').nest(construct!(b, c));
    let parser = construct!(adj, d).to_options();

    let r = parser.run_inner("-a 10").unwrap_err().unwrap_stderr();
    assert_eq!(r, "expected '-b=B', and more\n");
}
...
fn adjacent_error_message_pos_many() {
    ...
    let c = positional::<usize>("C");
    let d = short('d').switch();
    let adj = short('a').nest(construct!(b, c)).many();
    let parser = construct!(adj, d).to_options();
...
fn adjacent_is_adjacent() {
    let b = positional::<usize>("B");
    let parser = short('a').nest(b).many().to_options();

    let r = parser.run_inner("-a -a 10 20").unwrap_err().unwrap_stderr();
...
fn adjacent_limits_commands() {
    let x = pure(()).to_options().command("a").lazy();
